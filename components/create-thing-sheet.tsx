...
import { X, Plus, Check, Pin } from "lucide-react";
...
export default function CreateThingSheet({ onSave, onClose, defaultType, prefillDate, prefillStartTime }: CreateSheetProps) {
  ...
  const fakeThing = { type, dueDate, eventDate, completed: false } as Thing;
      ...
      <div className="flex-1 overflow-y-auto p-4" onClick={() => setActivePopup(null)}>
        ...
        <div className="relative overflow-visible" onClick={(e) => e.stopPropagation()}
            ...
            <div className="relative">
              <button onClick={() => togglePopup("circle")}
                ...
                {circle ? (
                  ...
                  <span className="text-[9px] font-bold" style={{ color: "rgba(0,0,0,0.25)" }}>@</span>
                )}
              </button>
              {activePopup === "circle" && (
          ...
          <div className="mt-2 space-y-2">
            ...
            {type === "password" && (
              <>
                ...
                <input type="password" className="w-full bg-transparent font-mono text-xs outline-none placeholder:opacity-40"
          ...
          <div className="mt-3 flex flex-wrap items-center gap-1">
            {tags.map((tag, idx) => (
              <span key={tag} className="flex items-center gap-0.5 px-1 py-px text-[9px] font-bold" style={{ background: "rgba(0,0,0,0.06)" }}>
                #{tag}
          ...
          <div className="mt-3 flex flex-wrap gap-3">
            <label className="flex items-center gap-1.5 text-[10px] font-bold" style={{ color: textColor, opacity: 0.5 }}>
              {type === "event" || isBirthday ? "Event:" : "Due:"}
              <input type="date" className="bg-transparent text-[10px] outline-none" style={{ color: textColor }}
                value={type === "event" || isBirthday ? eventDate : dueDate}
                onChange={(e) => { if (type === "event" || isBirthday) setEventDate(e.target.value); else setDueDate(e.target.value); }} />
            </label>
          </div>

          {/* CHECKBOX (lower-left) -- prominently visible for task-like types */}
          <div className="absolute bottom-2 left-3">
            {(type === "task" || type === "list" || type === "reminder") && (
              <div className="flex h-5 w-5 items-center justify-center rounded-sm border-2"
                style={{ borderColor: "rgba(0,0,0,0.15)", background: "rgba(0,0,0,0.02)" }}>
              </div>
            )}
          </div>

          {/* TYPE ICON (lower-right) -- tappable, opens grid picker */}
